...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create WhatsApp message text and open chat link
    const whatsappText = `Hi, I'm ${formData.name || 'a visitor'}.\nEmail: ${formData.email}\nSubject: ${formData.subject}\nMessage: ${formData.message}`;
    const whatsappUrl = `https://wa.me/918838006696?text=${encodeURIComponent(whatsappText)}`;
    window.open(whatsappUrl, '_blank');

    try {
      // Optional backend API call, but failure doesn't block WhatsApp flow
      ...
      if (response.ok) {
        ...
        toast({
          title: "Message queued for WhatsApp",
          description: "WhatsApp chat opened, even though the backend message endpoint failed.",
          variant: "default"
        });
      }
    } catch (error) {
      toast({
        ...
        description: "Unable to save via backend, but WhatsApp chat is ready.",
        variant: "default"
      });
    } finally {
          ...
          <div className="space-y-8">
            <div className="card-soft p-8 hover-subtle">
              ...
              <p className="text-muted-foreground mb-8 leading-relaxed">
                I’m always open to discussing new opportunities, interesting projects, or simply having a conversation about technology and development. Feel free to reach out via email or send me a message on WhatsApp. Let’s create something amazing together!
              </p>
